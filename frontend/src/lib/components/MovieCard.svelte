<script>
  ...
  let ringColor = $derived(
    matchPercent >= 85 ? '#c9a84c' :   // gold   — sangat cocok
    matchPercent >= 70 ? '#52c07a' :   // hijau  — cocok
    matchPercent >= 50 ? '#6a9fd8' :   // biru   — cukup
    '#7a7a8a'                          // abu    — kurang
  );
  ...
  const fallbackColors = [
    '#1a1a2e', '#16213e', '#0f3460', '#1b1b2f', '#2d132c', '#1c1c3a'
  ];
...
<article class="movie-card" style="--ring-color: {ringColor}">
  ...
  <div class="card-info">
    ...
    <div class="match-label" style="color:{ringColor}">
      {matchPercent >= 85 ? '★ Perfect Match' : matchPercent >= 70 ? '✦ Great Match' : '· Good Pick'}
    </div>
  </div>
</article>
...
.movie-card:hover {
  ...
  box-shadow: 0 16px 40px rgba(0,0,0,0.6), 0 0 0 1px var(--ring-color);
