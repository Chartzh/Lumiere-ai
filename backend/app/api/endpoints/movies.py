...
# >>> FIX A: loader rebuild+load_weights
from app.architecture import load_ncf_model

router = APIRouter()

# In-Memory Cache untuk data TMDB (Mitigasi Rate Limit)
TMDB_CACHE = {}
...
# >>> FIX B: helper fallback deterministik, dipakai BAIK saat DB error
# MAUPUN saat profil tidak ditemukan (DB Supabase masih kosong/seeding).
def resolve_demo_user_state(user_id: int):
    """
    Mengembalikan (is_new_user, genres) untuk kondisi tanpa data profil riil.
    user_id == 1 -> Old User (uji NCF + MMR)
    user_id == 2 -> New User dengan genre onboarding (uji Content-Based)
    user_id >= 3 -> New User skip onboarding (uji Popularity)
    """
    if user_id == 1:
        return False, []
    elif user_id == 2:
...
def generate_recommendations(user_id: int, db: Session):
    ...
    if is_mock_mode:
        ...
        try:
            ...
            # >>> FIX A: build + load_weights (anti error deserialisasi InputLayer)
            model = load_ncf_model(dest_path)
    ...
    try:
        ...
        if user:
            ...
            # >>> FIX B: profil TIDAK ditemukan (DB Supabase kosong/seeding awal).
            # Query sukses tapi kosong TIDAK lagi otomatis jadi New User;
            # pakai fallback deterministik yang sama dengan kondisi DB error.
            print(f"=== [DB EMPTY/SEEDING] Profil user_id={user_id} belum ada. Pakai fallback deterministik. ===")
            is_new_user, genres = resolve_demo_user_state(user_id)
    except Exception as e:
        print(f"=== [DB CONNECTION ERROR] Fallback to Mock User State: {str(e)} ===")
        # >>> FIX B: pakai helper yang sama agar perilaku konsisten
        is_new_user, genres = resolve_demo_user_state(user_id)
